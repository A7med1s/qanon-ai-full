...
import { FaExclamationTriangle, FaPaperclip, FaDownload, FaSpinner, FaCopy } from 'react-icons/fa';
...
const RiskAnalysisPage = () => {
  ...
  const headers = {
    ...
    try {
        const response = await axios.post(import.meta.env.VITE_BACKEND_URL+'/api/risk-analysis/analyze-risks', formData, {
            headers: { ...headers },
        ...
        if (outputFormat === 'docx') {
            ...
            link.href = url;
            link.setAttribute('download', `risk_analysis_${Date.now()}.docx`);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="contractText" className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">
              {t('enter_contract_text_for_analysis')}
            </label>
            <textarea
              id="contractText"
              ...
              placeholder={t('paste_contract_analysis_placeholder')}
              value={contractText}
          ...
          >
            {loading ? (
              ...
              <>
                <FaExclamationTriangle className="mx-2" /> {t('analyze_button')}
              </>
            )}
          </button>
        </form>
